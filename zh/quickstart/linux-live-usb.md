# Linux Live USB 启动盘(开源方案)

> 把任意 x86_64 电脑变成 AI 编程工作站:插上 U 盘,从 USB 启动,进入 Ubuntu 桌面后运行 U-Hermes。

本页直接对应 [u-hermes-oss/linux/](https://github.com/dongsheng123132/u-hermes/tree/main/linux) 的开源 Linux 盘启动方案,采用 Ventoy + Ubuntu Live + casper-rw 持久化的四步制盘结构。
...
在 Windows 上以普通 PowerShell 进入 [u-hermes 仓库](https://github.com/dongsheng123132/u-hermes) 的 `linux/` 目录:
...
A: 开源方案需要自己制盘、联网安装、配置 API Key 和排错;U-Hermes 马盘商业版是 Windows 成品 U 盘,内置 U-Claw + U-Hermes、图形启动器、账户体系、初始额度和售后。商业版请看 [Windows 商业版页](/zh/install/windows-portable)。
